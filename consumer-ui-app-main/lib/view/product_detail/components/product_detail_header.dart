...
class ProductDetailHeader extends StatelessWidget {
            ...
            if (navbar['center'] != null && navbar['center'].length > 0) ...[
              Row(children: [
                for (var item in navbar['left']) ...[
                  ...
                  ] else if (item['kind'] == 'product-title') ...[
                    Expanded(child: productTitleContainer(header, item))
  ...
  Widget iconContainer(header, item) {
    return InkWell(
        onTap: () {
          if (header['options'][item['key']]['screenId'] == 'share') {
  ...
  Container productTitleContainer(header, item) {
    return Container(
        padding: themeController.headerStyle(
            'padding', header['style'][item['key']]['padding']),
        child: Text(

...
class CollectionV1ControllerV2 extends GetxController with BaseController {
  ...
  var selectedCatalogOption = 'email'.obs;
  TextEditingController? catalogEmailController, catalogWhatsappController;
  var collectionData = new ProductCollectionV1VM(
    collection: null,
    count: null,
    catalogUrl: null,
    multiBooking: null,
  ...
  var selectedSortValue = Rx<sortSettingList?>(null);
  ...
  void onInit() {
    ...
    collectionRepo = CollectionRepo();
    wishlistRepo = WishListRepo();
    catalogEmailController = TextEditingController(text: '');
    catalogWhatsappController = TextEditingController(text: '');
    wishListRepo = WishListRepo();
  ...
  Future<void> getProductList(
    ...
    // print("template.value --------------->>>>> ${template.value}");
    // print(
    ...
    var input = {
      "collectionId": categoryLink,
      "pageNo": page,
      "filter": filter,
      "oneFilter": useV1 == true ? oneFilterValue : null,
    };
    var result = await collectionRepo!.getProductsByCollection(input);
    // print("result=================------------------->>>> ${result.toJson()}");
    if (result != null) {
      var productListDetails = productCollectionV1VMFromJson(result);
      ...
      print('schhhhhhhhh ${productListDetails.sortSetting}');
  ...
  Future<void> openFilterV2() async {
    var input = {
      "collectionUrl": selectedCategoryLink,
    };
    ...
    if (result != null) {
      ...
      //       productListDetails.collection!.name.toString();
      //   newData = productListDetails.product;
      // if (productListDetails.filters != null) {
      filterAttributeValue.value = productListDetails.filters!;
      // }
  ...
  Future<void> openTopFilter(BuildContext context, controller) async {
    ...
    print("template.value openTopFilter---------->>>>> ${template.value}");
  ...
  Future downloadProductCatalogByProductIds() async {
    showLoading("Loading...");
    var result = await collectionRepo!
        .downloadProductCatalogByProductIds(selectedProducts.value);
    hideLoading();
    if (result != null && result['template'] != null) {
      var splittedData = result['template'].split('/');
      var fileName = splittedData.last;
      await CommonService().downloadFile(fileName, result['template']);
    } else {
      showMessage("Failed to download");
    }
  }
  ...
  Future shareCatalog() async {
    showLoading("Loading...");
    var input = {
      "input": jsonEncode({
        "type": selectedCatalogOption.value,
        "value": selectedCatalogOption.value == 'whatsapp'
            ? catalogWhatsappController!.text
            : catalogEmailController!.text
      }),
      "productIds": selectedProducts.value
    };
    var result = await collectionRepo!.shareProductCatalogByProductIds(input);
    hideLoading();
    Get.back();
    if (result != null) {
      snackMessage(result['message']);
      selectedCatalogOption.value = 'email';
      catalogEmailController!.text = '';
      catalogWhatsappController!.text = '';
    }
  }

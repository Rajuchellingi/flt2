...
class SearchPageController extends GetxController with BaseController {
  ...
  void _onScroll() {
    ...
    _scrollDebounce = Timer(_scrollDebounceDelay, () {
      ...
      if (delta <= 200 && !loading.value && !allLoaded) {
        paginationFetch();
      }
    });
  }
  ...
  Future getSearchProducts() async {
    var productIdentifiers = themeController.productBadge != null
        ...
        : [];
    var result = await searchRepo!.getSearchProducts(searchText,
        pageInfo.value.endCursor, selectedSort.value, productIdentifiers);

    if (result != null) {
      var productListDetails = searchProductVMFromJson(result);
      pageInfo.value = productListDetails.pageInfo!;
      newData = productListDetails.products;
      if (newData.isNotEmpty) await assignNectorReviews(newData);
    }
  }

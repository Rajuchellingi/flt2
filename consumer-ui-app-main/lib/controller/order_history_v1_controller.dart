...
class OrderHistoryV1Controller extends GetxController with BaseController {
  OrderRepo? orderRepo;
  var orderList = [].obs;
  var pageIndex = 1;
  var pageLimit = 20;
  ...
  var order = new MyOrdersVM(orderData: [], count: 0, totalPages: 0).obs;
  ...
  Future getOrderList(pageNo, limit) async {
    var result = await orderRepo!.getOrderListByUser(pageNo, limit);
    if (result != null) {
      var orderData = myOrdersVMFromJson(result);
  ...
  Future navigateToOrderDetails(orderItem) async {
    await Get.toNamed('/orderDetail', arguments: {"id": orderItem.sId});
  }

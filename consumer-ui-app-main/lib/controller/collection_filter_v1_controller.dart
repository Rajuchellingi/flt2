...
class CollectionFilterV1Controller extends GetxController {
  ...
  updateInitialData(filterAttributeValue) {
    ...
    print(
        "filterAttribute.value: ${filterAttribute.value.map((e) => e.toJson()).toList()}");
  }

  Future getCategoryAttributeWithSet(
      collectionFilter, filterAttributeValue, List filter) async {
    // print(
    //     "collectionFilterApplyValues---------->>>> ${collectionFilter.toString()}");
    isLoading.value = true;
    // print(
    //     "collectionFilterApplyValues---------->>>> ${collectionFilter.map((e) => e.toJson()).toList()}");
    var collectionFilterApplyValues = {"attribute": collectionFilter['filter']};
    categoryAttribute.value =
        collectionAttributeVMMFromJson(collectionFilterApplyValues);
    print(
        "categoryAttribute--->>>getCategoryAttributeWithSet ${categoryAttribute.value.toJson()}");
  ...
  void updateFilterV1(String fieldName, String fieldValue, c_controller) {
    // Remove all previous filters for this fieldName
    selectedFilterAttr.removeWhere((element) => element.fieldName == fieldName);

    // Add only the new selected one

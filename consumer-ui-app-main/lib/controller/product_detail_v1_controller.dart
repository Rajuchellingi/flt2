...
class ProductDetailV1Controller extends GetxController with BaseController {
  ...
  Future getProductDetails(packDetail, isDefault, List variants) async {
    // print("template.value ------>>>> ${template.value.toJson()}");
    // print("template.value ------------->>>> ${template.value.map((e) => e.toJson()).toList()}");
    ...
    if (template.value['layout']['blocks'] != null) {
      for (var block in template.value['layout']['blocks']) {
        if (block['instanceId'] == "design6") {
          print("hloooooooooooooooooooo111111111111111111");
          if (product.value.packVariant != null &&
              product.value.packVariant!.isNotEmpty) {
          ...
          // print("hloooooooooooooooooooo222222222222222222");
  ...
  void selectedColorVariantsbyIndex(fieldValue, field, context, images) {
    ...
    if (attributeFieldValues.isNotEmpty) {
      ...
      print("defaultField===>>> ${defaultField.toString()}");
  ...
  void selectedColor(fieldValue, images) {
    ...
    var field = {
      ...
      "imageId": fieldValue['variantImage']['imageId'],
      "imageName": fieldValue['variantImage']['imageName'],
  ...
  void updateDefaultColor(fieldValue) {
    ...
    if (attributeFieldValues.isNotEmpty) {
      ...
      defaultField['isCustomizable'] = fieldValue.variantImage.isCustomizable;
      defaultField['isAssorted'] = fieldValue.variantImage.isAssorted;
      defaultField['setQuantity'] = fieldValue.variantImage.setQuantity;
  ...
  void updateSummaryProduct(context) async {
    print("product.value ---->>> ${product.value.sId}");
    var uToken = storage.read('utoken');
    if (uToken != null) {
      // KeyboardUtil.hideKeyboard(context);
      var data = {};
